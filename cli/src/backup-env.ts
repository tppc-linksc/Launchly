const SENSITIVE_ENV_KEY =
  /(?:PASSWORD|SECRET|TOKEN|PRIVATE_KEY(?:_BASE64)?|AUTH_JSON|ENCRYPTION_(?:KEY|PREVIOUS_KEYS)|DATABASE_URL)$/i;
...
export function hasRequiredRestoreKeys(contents: string): boolean {
  ...
  return ['LAUNCHLY_DB_PASSWORD', 'LAUNCHLY_JWT_SECRET', 'LAUNCHLY_ENCRYPTION_KEY'].every((key) => keys.has(key));
}
