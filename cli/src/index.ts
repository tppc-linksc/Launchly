...
program.name('launchly').description('Launchly CLI - 自托管部署平台命令行工具').version('0.2.0');
...
program.command('restore <file>').description('从备份恢复').option('--force', '跳过确认提示').action(cmdRestore);
...
program.command('shell <service>').description('进入指定服务容器的交互式 shell').action(cmdShell);
