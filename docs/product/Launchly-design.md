...
Launchly 是一个面向个人开发者和小团队的自托管自动化部署测试平台。它帮助团队把 Git 项目快速部署到测试、预发和生产环境，并把测试用例、测试结果、问题分派、权限、通知和发布记录统一管理起来。

Launchly 的目标不是替代 GitHub、GitLab 或完整企业 DevOps 平台，而是为小团队提供一套可以下载、本地部署、开箱即用的轻量发布测试系统。

### 1.3 部署形态

Launchly 支持两种主要使用方式：

- **本地自托管**：个人或小团队下载后部署在本地服务器、NAS、内网机器或私有云环境。
- **公网部署**：用户可以将 Launchly 部署到公网服务器，供远程团队协作使用。

产品应优先保证自托管场景体验清晰、依赖可控、安装流程简单。最终交付形态应支持命令行一键部署，用户可以通过 `launchly` CLI 完成安装、初始化、启动、升级、备份和诊断。

一键部署的目标是：用户不需要自己安装或配置数据库、队列、对象存储、应用运行服务等内部依赖。Launchly 必要运行组件应随安装包自动拉起并完成初始化。用户只需要提供极少数必须信息，例如最高权限管理员账号、管理员密码，以及公网部署时可选的域名和邮箱。

### 1.4 核心价值

- **开发人员**：绑定仓库后可以按分支或 commit 一键发布到测试环境，查看构建日志和部署状态。
- **测试人员**：围绕具体版本执行测试用例，记录测试结果，发现问题后指派对应成员修复。
- **负责人/管理员**：管理成员、权限、环境变量、发布门禁和上线记录，保证发布过程可追踪。
- **小团队**：用一个轻量系统串联代码、部署、测试、修复、复测和上线闭环。

### 1.5 产品边界

Launchly 聚焦“项目部署测试协作闭环”，第一阶段不追求成为完整代码托管平台、复杂 CI/CD 编排系统或企业级 ITSM 平台。
...
Launchly 采用 `Workspace -> Project -> Environment -> Release` 的组织模型。
...
Workspace 是团队协作的最上层空间。一个本地部署实例可以默认创建一个 Workspace，后续可扩展支持多个 Workspace。

### 2.2 用户角色

| 角色 | 定位 | 核心能力 |
| --- | --- | --- |
| Owner | Workspace 拥有者 | 管理 Workspace、成员、项目、环境、权限、所有发布操作 |
| Admin | 管理员 | 管理项目、成员、环境配置、发布流程 |
| Developer | 开发人员 | 绑定仓库、触发测试环境部署、查看日志、处理问题 |
| Tester | 测试人员 | 管理测试用例、执行测试、创建问题、复测问题、推动预发 |
| Viewer | 只读成员 | 查看项目、环境、部署记录、测试结果和问题 |
