...
function asKeywords(...values: any[]): string[] {
  return values.filter((v) => v !== null && v !== undefined && v !== '').map((v) => String(v).toLowerCase());
}
...
export function useGlobalSearch(options: UseGlobalSearchOptions = {}) {
  ...
  const results = computed<SearchResult[]>(() => {
    ...
    return filtered.sort((a, b) => (rank.get(a.category) ?? 99) - (rank.get(b.category) ?? 99)).slice(0, maxResults);
  });
