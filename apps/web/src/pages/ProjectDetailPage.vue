  ...
  <div v-if="project">
    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px">
      ...
      <div>
        ...
        <el-button style="margin-left: 8px" @click="$router.push(`/projects/${project.id}/deploy-targets`)"
          >部署目标</el-button
        >
        <el-button
          v-if="canDeploy"
      ...
      <el-col :span="16">
        <el-card header="项目信息" style="margin-bottom: 16px">
          <el-descriptions :column="2" size="small">
            ...
            <el-descriptions-item label="初始化管理员">{{
              project.bootstrapAdminEnabled ? '已配置（首次部署时创建）' : '未配置'
          ...
          <el-row :gutter="16">
            <el-col :span="8" v-for="env in environments" :key="env.id">
              <el-card size="small" :header="env.name" :style="{ borderTop: `3px solid ${envColor(env.type)}` }">
                ...
                <el-tag :type="env.status === 'active' ? 'success' : 'info'" style="margin-top: 4px">{{
                  env.status === 'active' ? '活跃' : '未激活'
                }}</el-tag>
                <div style="margin-top: 8px">
                  <el-button
                    v-if="canDeploy && (env.type === 'TEST' || env.type === 'STAGING')"
                    size="small"
                    type="primary"
                  ...
                  >
                  <el-button
                    v-else-if="!canDeploy && (env.type === 'TEST' || env.type === 'STAGING')"
                    size="small"
                    disabled
                    >无部署权限</el-button
                  >
      ...
      <el-col :span="8">
        <el-card header="环境变量" size="small" style="margin-bottom: 16px">
          <p style="color: #8c8c8c; margin: 0">
            前往 <a @click="$router.push(`/environments?projectId=${project.id}`)">环境管理</a> 页面管理变量
          </p>
        </el-card>
        <el-card header="最近部署" size="small">
          ...
          >
            <el-tag :type="deployTagType(d.status)" size="small" style="margin-right: 6px">{{
      ...
      <el-form label-position="top">
        <el-form-item label="目标环境">
          <el-select v-model="deployForm.environmentId" placeholder="选择环境">
            <el-option
              v-for="env in deployableEnvs"
              :key="env.id"
              :value="env.id"
              :label="`${env.name} (${envTypeMap[env.type] || env.type})`"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="部署目标" required>
          ...
          <div style="font-size: 12px; color: #8c8c8c; margin-top: 4px">
            在云服务器上部署请先在右上角「部署目标」里添加并「测试连接」。环境管理页请保持「本地部署」——表示 Worker
            用本机 Docker 构建后再把镜像推到目标机，与「远程」旧选项无关。
          </div>
        </el-form-item>
    ...
    <el-dialog v-model="showEdit" title="编辑项目">
      <el-form label-position="top">
        ...
        <el-alert
          ...
          title="Launchly 不直接写业务数据库。首次部署时会把以下信息短暂注入应用容器，并执行你声明的初始化命令。密码加密保存且不再回显。"
        />
        ...
        <template v-if="editForm.bootstrapAdminEnabled">
          <el-form-item label="容器内初始化命令" required>
            ...
            <div style="font-size: 12px; color: #909399; margin-top: 4px">
              命令在 app 容器中执行，可读取 LAUNCHLY_BOOTSTRAP_ADMIN_USERNAME、EMAIL、PASSWORD。
            </div>
          </el-form-item>
          ...
          <el-form-item label="管理员初始密码" :required="!project?.bootstrapAdminEnabled">
            <el-input
              v-model="editForm.bootstrapAdminPassword"
              type="password"
              show-password
              autocomplete="new-password"
              :placeholder="project?.bootstrapAdminEnabled ? '留空则保持已有加密密码' : '至少 8 位，需包含字母和数字'"
            />
          </el-form-item>
        </template>
      </el-form>
...
const deployableEnvs = computed(() => environments.value.filter((e: any) => e.type === 'TEST' || e.type === 'STAGING'));

const canDeployProject = computed(() => {
  if (!project.value) return false;
  return Boolean(
    project.value.repositoryUrl || project.value.imageReference || project.value.templateId === 'static-blog',
  );
});
...
async function doEdit() {
  ...
  try {
    if (
      editForm.bootstrapAdminEnabled &&
      (!editForm.bootstrapAdminCommand.trim() ||
        (!editForm.bootstrapAdminUsername.trim() && !editForm.bootstrapAdminEmail.trim()))
    ) {
