<template>
  <div>
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 24px; flex-wrap: wrap">
      ...
      <el-tag :type="release?.gateStatus === 'PASSED' ? 'success' : 'danger'">{{
        release?.gateStatus === 'PASSED' ? '通过' : release?.gateStatus === 'FAILED' ? '失败' : '未知'
      }}</el-tag>
      <div style="flex: 1" />
      <el-button v-if="release?.status === 'READY'" type="primary" @click="handlePublish" :loading="publishing"
        >发布</el-button
      >
    </div>
    ...
    <el-table :data="gateResults" row-key="gateName" size="small" style="margin-bottom: 24px">
      ...
      <el-table-column label="操作" width="80">
        <template #default="{ row }">
          <el-button v-if="!row.passed && release?.status !== 'PUBLISHED'" link size="small" @click="handleExempt(row)"
            >豁免</el-button
          >
        </template>
      </el-table-column>
