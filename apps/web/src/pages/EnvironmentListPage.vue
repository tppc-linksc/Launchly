  ...
  <div>
    ...
    <el-table :data="filteredEnvs" row-key="id" v-loading="loading">
      <el-table-column prop="projectId" label="所属项目" width="150">
        <template #default="{ row }">
          <a
            ...
            >{{ projectNameMap[row.projectId] || '未知' }}</a
          >
        </template>
      </el-table-column>
      ...
      <el-table-column prop="type" label="类型" width="100">
        <template #default="{ row }">
          <el-tag :type="row.type === 'TEST' ? 'primary' : row.type === 'STAGING' ? 'warning' : 'danger'">{{
            envTypeMap[row.type] || row.type
          }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="varModalOpen" :title="'环境变量 — ' + (selectedEnv?.name || '')" width="640px">
      ...
      <el-form :model="newVar" inline @submit.prevent="addVar">
        ...
        <el-form-item
          ><el-switch v-model="newVar.sensitive" active-text="敏感" inactive-text="普通" inline-prompt
        /></el-form-item>
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="域名（自动 Nginx 路由）">
          ...
          <div style="color: #909399; font-size: 12px; margin-top: 6px">
            填写域名后，目标机自动运行共享 Nginx 代理容器并路由至此环境；多个项目可共用 80 端口。
          </div>
        </el-form-item>
...
function openEditModal(env: any) {
  ...
  editForm.branchPattern =
    env.branchPattern || (env.type === 'TEST' ? 'develop' : env.type === 'PRODUCTION' ? 'main' : '');
...
async function loadEnvs() {
  ...
  try {
    ...
    const targetProjects = qp ? projects.value.filter((p: any) => p.id === qp) : projects.value;
