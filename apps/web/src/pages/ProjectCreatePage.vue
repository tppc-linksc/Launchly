<template>
  <div class="page">
    ...
    <el-alert v-if="item?.availability !== 'DEPLOYABLE'" type="warning" :closable="false" show-icon style="margin-bottom: 18px"
      ...
      <el-form :model="form" label-position="top" @submit.prevent="onSubmit">
        ...
        <template v-if="usesGit">
          ...
          <el-form-item label="仓库地址" required>
            <el-input v-model="form.repositoryUrl" :placeholder="form.sourceType === 'DEPLOY_KEY' ? 'git@github.com:org/repo.git' : 'https://github.com/org/repo.git'" />
          </el-form-item>
          ...
          <template v-if="form.sourceType === 'DEPLOY_KEY'">
            ...
            <el-form-item label="仓库 Deploy Key（私钥）" required>
              <el-input v-model="form.repositoryPrivateKey" type="textarea" :rows="5" autocomplete="off" placeholder="-----BEGIN OPENSSH PRIVATE KEY-----" />
            </el-form-item>
        ...
        <el-alert v-if="form.topology !== 'SINGLE_SERVICE' && item.availability === 'DEPLOYABLE'" type="info" :closable="false" style="margin-bottom: 18px"
        ...
        <template v-if="item.availability === 'DEPLOYABLE' && form.sourceType !== 'OCI_IMAGE'">
          ...
          <el-form-item label="构建命令"><el-input v-model="form.buildCommand" placeholder="npm run build" /></el-form-item>
        ...
        <template v-if="item.availability === 'DEPLOYABLE'">
          ...
          <el-form-item label="健康检查路径"><el-input v-model="form.healthCheckPath" placeholder="/health" /></el-form-item>
          <el-form-item label="容器端口"><el-input-number v-model="form.defaultPort" :min="1" :max="65535" style="width: 100%" /></el-form-item>
        </template>
    ...
    <el-empty v-else-if="!catalogLoading" description="资源类型不存在或已下线"><el-button type="primary" @click="router.push('/resources/new')">返回资源目录</el-button></el-empty>
...
async function onSubmit() {
  ...
  try {
    ...
    if (form.sourceType === 'DEPLOY_KEY') payload.repositoryCredential = { privateKey: form.repositoryPrivateKey, hostKey: form.repositoryHostKey }
    const res = await createProject(payload)
    router.push(`/projects/${res.data.id}`)
  } catch (e: any) { error.value = e.response?.data?.message || '保存失败' } finally { loading.value = false }
}

onMounted(async () => {
  catalogLoading.value = true
  try { catalog.value = (await fetchResourceCatalog()).data || []; applyItem(item.value) } catch { error.value = '无法加载资源目录' } finally { catalogLoading.value = false }
})
</script>

<style scoped>
.page { max-width: 760px; } .head { display: flex; justify-content: space-between; gap: 20px; align-items: center; margin-bottom: 20px; } .head h2 { margin: 6px 0 0; } .head p { color: #6b7280; margin: 6px 0 0; }
.form-card { border-radius: 12px; } .hint { color: #909399; font-size: 12px; margin-top: 6px; line-height: 1.4; }
@media (max-width: 700px) { .head { align-items: flex-start; flex-direction: column; } }
</style>
