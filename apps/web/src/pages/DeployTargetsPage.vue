      ...
      <el-table :data="filteredTargets" v-loading="loading" row-key="id" size="default">
        ...
        <el-table-column prop="status" label="状态">
          <template #default="{ row }">
            <el-tag
              :type="
                row.status === 'VERIFIED' || row.status === 'CONNECTED'
                  ? 'success'
                  : row.status === 'FAILED'
                    ? 'danger'
                    : 'warning'
      ...
      <el-form label-position="top" :model="form">
        ...
        <el-form-item label="Launchly 工作目录">
          ...
          <span style="font-size: 11px; color: #8c8c8c"
            >Linux 默认目录。NAS 可填写已创建且 SSH 用户与 Docker 都能读写的共享目录，例如挂载卷中的
            <code>/volume1/launchly</code>；不使用临时目录。</span
          >
        </el-form-item>
        ...
        <el-form-item label="服务器 Host Key" required>
          <el-input
            ...
            type="textarea"
            :rows="2"
            placeholder="例如：ssh-ed25519 AAAA..."
            spellcheck="false"
          />
          <span style="font-size: 11px; color: #8c8c8c"
            >从目标机可信来源确认后填写；Launchly 不会自动信任未知主机。NAS 的 Host Key 路径可能不同于 Ubuntu。</span
          >
        </el-form-item>
      </el-form>
