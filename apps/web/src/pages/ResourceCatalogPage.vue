<template>
  <div>
    ...
    <el-alert type="info" :closable="false" show-icon style="margin-bottom: 20px">
      <template #title
        >应用资源可以选择公开 Git、GitHub App、Deploy Key 或不可变 OCI
        镜像；有状态数据库和一键模板会先保存受控配置，待其备份、恢复和升级执行器完成后才允许发布。</template
      >
    </el-alert>
    <div v-loading="loading">
      <section v-for="section in sections" :key="section.key" class="section">
        ...
        <div class="catalog-grid">
          <button
            v-for="item in itemsByCategory(section.key)"
            :key="item.id"
            class="resource-card"
            @click="select(item)"
          >
...
const filtered = computed(() => {
  ...
  return needle
    ? items.value.filter((item) => `${item.title} ${item.description}`.toLowerCase().includes(needle))
    : items.value;
});
function itemsByCategory(category: string) {
  return filtered.value.filter((item) => item.category === category);
}
function select(item: any) {
  router.push({ path: '/projects/create', query: { resource: item.id } });
}
...
onMounted(async () => {
  loading.value = true;
  try {
    items.value = (await fetchResourceCatalog()).data || [];
  } finally {
    loading.value = false;
  }
});
