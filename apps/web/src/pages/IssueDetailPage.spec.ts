...
const ISSUE_OPEN = {
  id: 'i1', projectId: 'p1', title: 'login broken',
  priority: 'P0', status: 'OPEN', assigneeId: null,
  createdAt: '2026-01-01', dueDate: null, fixedCommitSha: null,
...
describe('IssueDetailPage', () => {
  ...
  it('ID.5 the FIXED transition (via "标记已修复" modal) includes fixedCommitSha', async () => {
    ...
    vi.mocked(transitionIssue).mockResolvedValue({ data: { ...FIXING, status: 'FIXED', fixedCommitSha: 'sha-1' } } as any)
    ...
    const commitInput = document.querySelector(
      'input[placeholder="输入修复 commit SHA"]',
