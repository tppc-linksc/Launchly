...
const DEPLOY_TARGETS = [{ id: 't1', name: 'prod-a', host: '10.0.0.1' }];
...
describe('ProjectDetailPage', () => {
  ...
  it('PD.11 the per-env color border uses the env type to set the top border color', async () => {
    ...
    const testCard = Array.from(document.querySelectorAll('.el-card')).find((c) =>
      (c.querySelector('.el-card__header')?.textContent || '').includes('测试'),
    );
    const stagingCard = Array.from(document.querySelectorAll('.el-card')).find((c) =>
      (c.querySelector('.el-card__header')?.textContent || '').includes('Staging'),
    );
    const prodCard = Array.from(document.querySelectorAll('.el-card')).find((c) =>
      (c.querySelector('.el-card__header')?.textContent || '').includes('生产'),
    );
  ...
  it('PD.14 doEdit rejects an empty project name with ElMessage.warning and does NOT call updateProject', async () => {
    ...
    const saveBtn = Array.from(dialog.querySelectorAll('button')).find((b) => b.textContent?.trim() === '保存') as
