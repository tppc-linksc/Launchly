<template>
  <div>
    ...
    <p style="color: #8c8c8c; margin-bottom: 24px">
      当前工作空间内的全部部署记录（不限于单个项目）。从项目详情触发部署后，可在此查看历史与状态。
    </p>
    <el-table
      :data="deployments"
      v-loading="loading"
      row-key="id"
      @row-click="(r: any) => $router.push(`/deployments/${r.id}`)"
    ...
    >
      ...
      <el-table-column label="操作" width="180">
        <template #default="{ row }">
          ...
          <el-button v-if="canDeploy && row.status === 'FAILED'" type="danger" link @click.stop="handleRedeploy(row)"
            >重新部署</el-button
          >
          <el-button
            v-if="canDeploy && row.status === 'SUCCEEDED' && row.commitSha"
            type="primary"
            link
...
async function handleRollback(record: any) {
  try {
    await ElMessageBox.confirm(`确定要回滚到 commit ${record.commitSha?.substring(0, 7)} 的版本吗？`, '回滚确认', {
      type: 'warning',
    });
