...
describe('GitRunner.execute - input gate rejects before any side effect', () => {
  ...
  it('rejects when branch contains a NUL byte', async () => {
    ...
    const result = await runner.execute(
      makeContext({ payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'main\0bad' } }),
    );
    expect(result.success).toBe(false);
  ...
  it('rejects when branch contains CR or LF', async () => {
    ...
    const r1 = await runner.execute(
      makeContext({ payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'ma\rbad' } }),
    );
    const r2 = await runner.execute(
      makeContext({ payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'ma\nbad' } }),
    );
  ...
  it('rejects when branch is exactly 256 chars and accepts 255 (KI-032 caller input gate)', async () => {
    ...
    const ok = await runner.execute(
      makeContext({ payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'a'.repeat(255) } }),
    );
    ...
    const bad = await runner.execute(
      makeContext({ payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'a'.repeat(256) } }),
    );
  ...
  it('rejects when commitSha contains NUL/CR/LF or exceeds 255 chars (KI-032 caller input gate)', async () => {
    ...
    for (const bad of ['abc\0def', 'abc\rdef', 'abc\ndef', 'a'.repeat(256)]) {
      const result = await runner.execute(
        makeContext({ payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'main', commitSha: bad } }),
      );
      expect(result.success).toBe(false);
      expect(result.errorMessage).toBe('仓库 URL / branch / commit 非法');
    }
  ...
  it('rejects when sourceType is not GIT_PUBLIC / GITHUB_APP / DEPLOY_KEY (after creating the workDir)', async () => {
    ...
    const runner = makeRunner(deps);
    const result = await runner.execute(
      makeContext({ payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'main', sourceType: 'WAT' } }),
    );
    expect(result.success).toBe(false);
...
describe('GitRunner.execute - GIT_PUBLIC path', () => {
  ...
  it('sanitizes stdout/stderr from a successful clone (CommandExecutor.sanitize is real)', async () => {
    ...
    deps.executor.execFile.mockResolvedValueOnce({
      stdout: 'connected with password=hunter2',
      stderr: 'leaked token=ghp_abcdefghijklmnopqrstuvwxyz',
      exitCode: 0,
    });
    const runner = makeRunner(deps);
    const result = await runner.execute(makeContext());
  ...
  it('non-zero exit: returns failure with sanitized stdout/stderr and propagates exit code', async () => {
    ...
    fsMock.mkdirSync.mockReturnValueOnce(undefined);
    deps.executor.execFile.mockResolvedValueOnce({
      stdout: 'auth fail password=hunter2',
      stderr: 'remote: bad token=ghp_zzzzzzzzzzzzzzzzzzzzz',
      exitCode: 128,
    });
    const runner = makeRunner(deps);
    const result = await runner.execute(makeContext());
  ...
  it('commitSha present: clone + fetch + detached checkout (FETCH_HEAD) + HEAD verification are called in order with exact argv (KI-033)', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'main', commitSha: 'abc1234' },
      }),
    );
    expect(result.success).toBe(true);
    ...
    expect(deps.executor.execFile.mock.calls[1][2]).toEqual({
      cwd: '/tmp/launchly-builds/deploy-1',
      timeout: 120,
      env: undefined,
    });
    ...
    expect(deps.executor.execFile.mock.calls[2][2]).toEqual({
      cwd: '/tmp/launchly-builds/deploy-1',
      timeout: 120,
      env: undefined,
    });
  ...
  it('commitSha present: HEAD mismatch fails closed with "实际 HEAD 与请求 commit 不一致" (KI-033)', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'main', commitSha: 'cafebabe' },
      }),
    );
    expect(result.success).toBe(false);
  ...
  it('commitSha present: fetch non-zero fails closed with "拉取失败" (KI-033)', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'main', commitSha: 'deadbee' },
      }),
    );
    expect(result.success).toBe(false);
  ...
  it('commitSha present: checkout non-zero fails closed with "检出失败" (KI-033)', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'main', commitSha: 'badcafe' },
      }),
    );
    expect(result.success).toBe(false);
...
describe('GitRunner.execute - GITHUB_APP source', () => {
  ...
  it('rejects an installation that is not operator-bound to the project workspace', async () => {
    ...
    fsMock.existsSync.mockReturnValueOnce(false);
    fsMock.mkdirSync.mockReturnValueOnce(undefined);
    deps.prisma.project.findUnique.mockResolvedValueOnce({
      id: 'proj-1',
    ...
    const result = await runner.execute(
      makeContext({
        payload: { projectId: 'proj-1', repositoryUrl: PUBLIC_URL, branch: 'main', sourceType: 'GITHUB_APP' },
      }),
    );
  ...
  it('rejects non-HTTPS repository URLs (http://)', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: {
          projectId: 'proj-1',
          repositoryUrl: 'http://github.com/acme/app.git',
          branch: 'main',
          sourceType: 'GITHUB_APP',
        },
      }),
  ...
  it('rejects HTTPS but non-github.com host', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: {
          projectId: 'proj-1',
          repositoryUrl: 'https://gitlab.com/acme/app.git',
          branch: 'main',
          sourceType: 'GITHUB_APP',
        },
      }),
  ...
  it('rejects github.com.evil.com subdomain hijack attempt', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: {
          projectId: 'proj-1',
          repositoryUrl: 'https://github.com.evil.com/acme/app.git',
          branch: 'main',
          sourceType: 'GITHUB_APP',
        },
      }),
  ...
  it('rejects completely invalid URLs (URL constructor throws, error propagates as failure)', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: { projectId: 'proj-1', repositoryUrl: 'not a url at all', branch: 'main', sourceType: 'GITHUB_APP' },
      }),
    );
    expect(result.success).toBe(false);
    expect(result.errorMessage).toBe('仓库 URL / branch / commit 非法');
...
describe('GitRunner.execute - DEPLOY_KEY source', () => {
  ...
  it('rejects when credentialType is not DEPLOY_KEY', async () => {
    ...
    fsMock.existsSync.mockReturnValueOnce(false);
    fsMock.mkdirSync.mockReturnValueOnce(undefined);
    deps.prisma.repositoryCredential.findUnique.mockResolvedValueOnce({
      id: 'cred-1',
      projectId: 'proj-1',
      credentialType: 'OAUTH',
      encryptedValue: 'v2:enc',
      hostKey: HOST_KEY,
    });
    const runner = makeRunner(deps);
    const result = await runner.execute(
  ...
  it('rejects unparseable SSH URL', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: { projectId: 'proj-1', repositoryUrl: '://broken', branch: 'main', sourceType: 'DEPLOY_KEY' },
      }),
    );
    expect(result.success).toBe(false);
    expect(result.errorMessage).toBe('仓库 URL / branch / commit 非法');
  ...
  it('scp-style URL writes task-local private key and known_hosts with mode 0600', async () => {
    ...
    expect(fsMock.writeFileSync.mock.calls[1]).toEqual([
      '/tmp/launchly-builds/.git-known-hosts-deploy-1',
      `${HOST} ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIABCDEFGHIJKLMNOPQRSTUVWXYZ\n`,
      { mode: 0o600 },
    ]);
  ...
  it('ssh:// URL: same credential + known_hosts + env, host parsed from URL', async () => {
    ...
    const result = await runner.execute(
      makeContext({
        payload: {
          projectId: 'proj-1',
          repositoryUrl: 'ssh://git@github.com/acme/app.git',
          branch: 'main',
          sourceType: 'DEPLOY_KEY',
        },
      }),
    ...
    expect(result.success).toBe(true);
    expect(fsMock.writeFileSync.mock.calls[1][1]).toBe(
      `${HOST} ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIABCDEFGHIJKLMNOPQRSTUVWXYZ\n`,
    );
  });
  ...
  it('secrets.decrypt throws: failure propagates and no execFile call', async () => {
    ...
    deps.secrets.decrypt.mockImplementationOnce(() => {
...
describe('GitRunner.execute - refId path boundary', () => {
  ...
  it('rejects unsafe DEPLOY_KEY refId before resolving credential paths', async () => {
    ...
    deps.prisma.repositoryCredential.findUnique.mockResolvedValueOnce({
      ...
      credentialType: 'DEPLOY_KEY',
      encryptedValue: 'v2:ciphertext',
      hostKey: 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIABCDEFGHIJKLMNOPQRSTUVWXYZ trusted-nas',
    });
