...
export class RemoteSshRunner {
  ...
  async execute(ctx: RunnerContext): Promise<RunnerResult> {
    ...
    try {
      ...
      if (![containerPort, externalPort].every(value => Number.isInteger(Number(value)) && Number(value) >= 1 && Number(value) <= 65535)) {
      ...
      if (target.projectId !== projectId) return this.failure('Deploy target does not belong to the deployment project');
  ...
  private async executeRollback(ctx: RunnerContext): Promise<RunnerResult> {
    ...
    try {
      ...
      const target = await this.prisma.deployTarget.findUnique({ where: { id: deployTargetId } });
      if (!target || target.authMethod !== 'KEY' || !target.hostKey || !SAFE_HOST.test(target.host) || !SAFE_USER.test(target.username)) {
        return this.failure('Rollback target is not safely configured');
      }
      // KI-004: 不允许跨项目 Target 进行回滚。
      if (target.projectId !== projectId) return this.failure('Rollback target does not belong to the deployment project');
  ...
  private async executeBootstrap(ctx: RunnerContext): Promise<RunnerResult> {
    ...
    try {
      ...
      if (target.projectId !== projectId) return this.failure('Bootstrap target does not belong to the deployment project');
      ...
      if (copy.exitCode !== 0) {
        await this.recordBootstrapResult(projectId, environmentId, ctx.refId, 'FAILED', 'Bootstrap credential transfer failed');
  ...
  private async recordBootstrapResult(projectId: string, environmentId: string, deploymentId: string, status: 'SUCCEEDED' | 'FAILED' | 'RUNNING', lastError: string | null) {
    try {
      await this.prisma.projectBootstrapRun.upsert({
        where: { projectId_environmentId: { projectId, environmentId } },
        create: { projectId, environmentId, deploymentId, status, lastError: lastError ?? undefined, completedAt: status === 'SUCCEEDED' ? new Date() : undefined },
        update: {
          deploymentId,
          status,
