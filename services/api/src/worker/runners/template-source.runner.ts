...
export class TemplateSourceRunner {
  async execute(ctx: RunnerContext): Promise<RunnerResult> {
    ...
    try {
      ...
      fs.writeFileSync(path.join(workDir, 'Dockerfile'), 'FROM nginx:1.27-alpine\nCOPY index.html /usr/share/nginx/html/index.html\nEXPOSE 80\n', { mode: 0o600 });
    ...
    } catch (error: any) { return this.failure(error?.message || 'Unable to create template source'); }
  }
  ...
  private escapeHtml(value: string) { return value.replace(/[&<>"']/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[char]!)); }
